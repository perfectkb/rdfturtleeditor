...
define("ace/mode/turtle_worker",["require","exports","module","ace/lib/oop","ace/worker/mirror","ace/ext/rdflib","ace/ext/fsm"], function(require, exports, module) {
    "use strict";
        ...
        this.onUpdate = function() {
            ...
            var fsm = StateMachine.create({
                ...
                error: function(eventName, from, to, args, errorCode, errorMessage, originalException) {
                    return 'syntax error: ' + eventName + ' :- ' + errorMessage;
                },
                events: [
                    { name: 'subject', from: 'start', to: 'predicate' },
                    { name: 'predicate', from: 'predicate', to: 'object' },
                    { name: 'object', from: 'object', to: ['end'] },
                    { name: 'prefix', from: 'start', to: 'namespace' },
                    { name: 'base', from: 'start', to: 'uri' },
                    { name: 'namespace', from: 'namespace', to: 'uri' },
                    { name: 'uri', from: 'uri', to: 'end' },
                    { name: 'end', from: 'end', to: 'start' }
                ]
            });
            var regex = new RegExp(/^<([a-zA-Z]+['s://'?'://']*?[a-zA-Z0-9-_./#?=]+)>/, 'g');
            var pname_nsregex = new RegExp(/^([a-zA-Z0-9-_]+[':'])/, 'g');
            var subjObjPredRegex = new RegExp(/^([a-zA-Z0-9-_]+[':'][a-zA-Z0-9-_]+)/, 'g');

            var emptyLineReg = new RegExp(/^[\s]*/, 'g');
            ...
            lines.forEach(function(line) {
                ...
                if (!line.trim().startsWith('#')) {
                    ...
                    words.forEach(function(curWord) {
                        var curWord = curWord.trim();
                        console.log("CURRENT WORD IS: " + curWord);
                        if (fsm.current === 'start' && (curWord === '@prefix' || curWord === 'PREFIX')) {
                            console.log('prefix identified')
                            if (fsm.prefix() !== 1) {
                                errors.push({
                                    row: curLine,
                                    column: lines[curLine].length - 1,
                                    text: fsm.current + ' is not valid',
                                    type: "error"
                                });
                            }
                        } else if (fsm.current === 'start' && (curWord === '@base' || curWord === 'BASE')) {
                            console.log('base identified')
                            if (fsm.base() !== 1) {
                                errors.push({
                                    row: curLine,
                                    column: lines[curLine].length - 1,
                        ...
                        } else if (fsm.current === 'start' && (subjObjPredRegex.test(curWord) || regex.test(curWord)) && curWord !== '@base' && curWord !== 'BASE' && curWord !== '@prefix' && curWord !== 'PREFIX') {
                            if (fsm.subject() !== 1) {
                                errors.push({
                                    row: curLine,
                                    column: lines[curLine].length - 1,
                        ...
                        } else if (regex.test(curWord)) {
                            console.log('uri identified');
                            if (fsm.uri() !== 1) {
                                errors.push({
                                    row: curLine,
                                    column: lines[curLine].length - 1,
                                    text: fsm.current + ' is not valid',
                                    type: "error",
                                    raw: fsm.current + " is not valid for uri"
                                });
                            }
                        } else if (fsm.current == 'namespace' && pname_nsregex.test(curWord)) {
                            console.log('namespace identified');
                            if (fsm.namespace() !== 1) {
                                errors.push({
                                    row: curLine,
                                    column: lines[curLine].length - 1,
                            ...
                            if (line !== '') {
                                errors.push({
                                    row: curLine,
                                    column: lines[curLine].length - 1,
                                    text: 'Invalid characters on this line',
                                    type: "error",
                                    raw: fsm.current + " is not valid"
                                });
                            }
